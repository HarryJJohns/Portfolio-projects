...
def search_book():
    ...
    result = cursor.fetchone()
    #output = Book(result[0], result[1], result[2], result[3], result[4])
    output = Book(result[0],result[1],result[2],result[3])
    print(output)
    # Drop virtual table to preserve memory and allow the search function to work again
    cursor.execute('''
    DROP TABLE booksV
    ''')
